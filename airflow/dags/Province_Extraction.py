...
def generate_output_file(province_table, **kwargs):
    data_interval_end = kwargs['data_interval_end'] - timedelta(days=1)
    output_file = f'{province_table}-{data_interval_end.strftime("%Y%m%d")}.csv'
...
def invoke_lambda(**kwargs):
    ...
    print(json_payload)
    api_url = f'https://4d1d8hk23f.execute-api.ap-southeast-2.amazonaws.com/dev?execution_date={output_file}'
    print(api_url)

    headers = {'Content-Type': 'application/json'}
    
    # Thực hiện POST request đến API Gateway
    response = requests.post(api_url, data=json.dumps(json_payload), headers=headers)
    # Kiểm tra response
...
def copy_csv_to_table(raw_table, **kwargs):
    hook = SnowflakeHook(snowflake_conn_id='snowflake_conn_raw')
    conn = hook.get_conn()
    cursor = conn.cursor()
...
with DAG(
    ...
    generate_output_file_task = PythonOperator(
        ...
        op_kwargs = {'province_table': 'province_table'},
    ...
    # Task 3: Truncate dữ liệu trong bảng Raw_Province phục vụ cho lần chạy Dag 
    truncate_task = PythonOperator(
        task_id = 'Truncate_Raw_Province',
        python_callable = Truncate_Raw_Table,
        op_kwargs = {'raw_table': raw_table},
        provide_context=True
    )

    # Task 4: Copy dữ liệu từ S3 vào bảng Stage_Table
    ...
    dbt_province_models = DbtTaskGroup(
        group_id="province_models",
    ...
    task_id='Trigger_DAG_Scrape_City',
    trigger_dag_id='Scrape_City'
    )

    generate_output_file_task >> invoke_lambda_task >> check_s3_file >> truncate_task >> copy_into_task >> sleep_task >> dbt_province_models >> Trigger_DAG_Task
